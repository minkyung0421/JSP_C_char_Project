...
<body>
		...
		<div style="font-family: NANUMBARUNGOTHICLIGHT;">
			...
			<%
				...
				if (!checkId.equals("")) {
					...
					try {
						if (!one.equals("") && !two.equals("") && !two_t.equals("") && !three.equals("")
								&& !three_t.equals("") && !four.equals("") && !five.equals("") && !five_t.equals("")
								&& !six.equals("") && !six_t.equals("") && !seven.equals("") && !eight.equals("")
								&& !nine.equals("") && !nine_t.equals("") && !ten.equals("")) {
 ...
 %>
 	...
 	writer.printf("<br>점수 : %s %n<br>", grade);
 		...
 		response.sendRedirect("mylist.jsp?send=" + c_count + "&result=" + result);
 	} else {
 %>
				<h3 style="margin-top: 30px">로그인 후에 결과 확인과 저장하기 이용이 가능합니다.</h3> <%
 	}
 %> <%
 	if (checkId.equals("")) {
 %> <a href="login.jsp"><input type="button"
					value="로그인" class="button"></a> <%
 	} else {
 %>
